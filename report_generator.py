...
def format_news_section(articles: list, news_summary: str) -> str:
    """뉴스 섹션 포맷"""
    if not articles:
        return "  No recent news available."
...
def generate_report(dcf_result: dict) -> str:
    ...
    articles = fetch_recent_news(ticker, company)
    news_summary = summarize_news(articles, company, ticker)
    news_section = format_news_section(articles, news_summary)
